<script>
	...
	async function fetchPhrases() {
		try {
			phrases = await Api.get(`/${elementType.toLowerCase()}/${element.id}/phrases`);
			loading = false;
		} catch (err) {
	...
	async function createPhrase() {
		try {
			const createdPhrase = await Api.post(
				`/${elementType.toLowerCase()}/${element.id}/phrases`,
				newPhrase
			);
	...
	async function updatePhrase(phrase) {
		...
		try {
			const updatedPhrase = await Api.put(`/phrases/${phrase.id}`, phrase);
	...
	async function generatePhrase() {
		...
		try {
			generating = true;
			const response = await Api.post('/phrases/generate', {
				phrasable_type: elementType,
				phrasable_id: element.id
			});
			phrases = [...phrases, response];
			// Since the response includes phrase_links, we can add it directly to our map
			if (response.phrase_links) {
				phraseLinksMap[response.id] = response.phrase_links;
				phraseLinksMap = phraseLinksMap;
			}
		} catch (err) {
			error = err?.message || 'Failed to generate phrase';
		} finally {
			generating = false;
		}
	}
					...
					<div class="phrases-list">
						{#each phrases as phrase (phrase.id)}
							<div use:inview={inviewOptions} on:inview_enter={() => fetchPhraseLinks(phrase.id)}>
								<Card class="mb-3">
									<CardBody>
										{#if editingPhrase?.id === phrase.id && $user?.admin}
											<Form
												on:submit={(e) => {
													e.preventDefault();
