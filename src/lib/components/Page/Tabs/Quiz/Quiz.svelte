<script>
	...
	// import ElementModal from '$lib/modals/videos/element.svelte';
...
<li class="quiz" class:has_video={quiz && quiz.preview}>
		...
		<div>
			...
			{#if showChoices}
				{#if quiz.choices.length === 0}
					<hr />
					<span class="choice">{quiz.element.title}</span>
				{/if}
	...
	{/if}
	{#if linkable}
		<span class="fa fa-link link" on:click={() => goto('/elements/' + quiz.quizable_id)} />
	{/if}
</li>
