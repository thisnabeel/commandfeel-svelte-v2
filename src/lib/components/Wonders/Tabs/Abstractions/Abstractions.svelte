<script lang="ts">
	...
	async function handleRemoveAbstraction(payload: { id: number }) {
		...
		wonder.abstractions = wonder.abstractions.filter((s) => s.id !== payload.id);
	}

	const addAbstraction = async () => {
		const response = await Api.post('/abstractions.json', {
			abstractable_id: wonder.id,
			abstractable_type: 'Wonder'
		});
		console.log(response);
		wonder.abstractions = [...wonder.abstractions, response];
		// console.log('fetch wonder', wonder);
		// fetchWonder(wonder.id);
	};

	const generateAbstraction = async () => {
		const response = await Api.post('/wonders/generate_abstraction.json', {
			id: wonder.id
		});
		...
		try {
			modals.open(QuestTitleModal, {
				...
				onTitleSubmit: async (title) => {
					...
					const quest = await API.post(`/wonders/${wonder.id}/quests`, {
						title: title,
...
<ul class="abstractions">
	...
	{/if}
	{#if wonder.abstractions.length > 0}
		{#each wonder.abstractions as abstraction}
			<li>
				<Abstraction
					{wonder}
					{abstraction}
