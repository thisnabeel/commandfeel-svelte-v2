<script>
	...
	async function fetchQuests() {
		try {
			quests = await API.get(`/wonders/${wonder.id}/quests`);
			loading = false;
		} catch (err) {
	...
	async function createQuest() {
		try {
			const createdQuest = await API.post(`/wonders/${wonder.id}/quests`, newQuest);
	...
	async function updateQuest(quest) {
		...
		try {
			const updatedQuest = await API.put(`/wonders/${wonder.id}/quests/${quest.id}`, quest);
	...
	async function deleteQuest(questId) {
		...
		try {
			await API.delete(`/wonders/${wonder.id}/quests/${questId}`);
	...
	async function openQuestSteps(quest) {
		...
		loadingSteps = true;
		try {
			questSteps = await API.get(`/wonders/${wonder.id}/quests/${quest.id}/quest_steps`);
		} catch (err) {
	...
	async function createStep() {
		...
		try {
			const res = await API.post(`/wonders/${wonder.id}/quests/${selectedQuest.id}/quest_steps`, {
				quest_step: newStep
			});
	...
	async function updateStep() {
		...
		try {
			await API.put(
				`/wonders/${wonder.id}/quests/${selectedQuest.id}/quest_steps/${editingStep.id}`,
	...
	async function deleteStep(stepId) {
		if (!selectedQuest) return;
		try {
			await API.delete(`/wonders/${wonder.id}/quests/${selectedQuest.id}/quest_steps/${stepId}`);
	...
	async function selectSuccessNodeOut(step) {
		...
		try {
			...
			await API.put(
				`/wonders/${wonder.id}/quests/${selectedQuest.id}/quest_steps/${successNodeIn.id}`,
				{
	...
	async function handleImageUpload(event, step) {
		...
		try {
			const res = await API.post(
				`/wonders/${wonder.id}/quests/${selectedQuest.id}/quest_steps/${step.id}/upload_image`,
				formData
			);
	...
	{:else}
		...
		<div class="quests-list">
			{#each quests as quest (quest.id)}
				<QuestCard {quest} wonderId={wonder.id} onDelete={deleteQuest} />
			{/each}
		</div>
	{/if}
</div>
